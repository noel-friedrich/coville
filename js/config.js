...
let VISIT_CHANCE = 0.142857 / 90 // https://www.youtube.com/watch?v=Ac08-99XPKw
let IMMUNETICKS = TICKS_PER_DAY * 7
let INCUBATIONTICKS = TICKS_PER_DAY * 3
let QUARANTINE_TICKS = TICKS_PER_DAY * 7
let TEST_CHANCE = 0.3
let TEST_FALSE_NEGATIVE_CHANCE = 0
let TEST_FALSE_POSITIVE_CHANCE = 0
let VACCINATION_LETHALITY_FACTOR = 0.1 // 10 times less likely to die
let VACCINATION_INFECTION_LENGTH_FACTOR = 0.5 // 50% shorter infection
let VACCINATION_INFECTION_CHANCE_FACTOR = 0.25 // 100% chance of infection
...
CONFIG_INFECTION_CHANCE.addEventListener("change", event => {
    ...
    for (let player of getAllPlayers()) {
        player.infectionChance = INFECTION_CHANCE
...
CONFIG_INFECTION_LENGTH.addEventListener("change", event => {
    IMMUNETICKS = TICKS_PER_DAY * parseInt(event.target.value)
    for (let player of getAllPlayers()) {
        player.maxImmuneTicks = IMMUNETICKS
        if (player.vaccinated)
            player.maxImmuneTicks *= VACCINATION_INFECTION_LENGTH_FACTOR
    }
})

CONFIG_INCUBATION_TIME.addEventListener("change", event => {
    INCUBATIONTICKS = TICKS_PER_DAY * parseInt(event.target.value)
    for (let player of getAllPlayers()) {
        player.maxIncubationTicks = INCUBATIONTICKS
        if (player.vaccinated)
            player.maxIncubationTicks *= VACCINATION_INFECTION_LENGTH_FACTOR
    }
})

CONFIG_QUARANTINE_TIME.addEventListener("change", event => {
    QUARANTINE_TICKS = TICKS_PER_DAY * parseInt(event.target.value)
})
...
CONFIG_VACCINATION_INFECTION_DURATION_FACTOR.addEventListener("change", event => {
    ...
    for (let player of getAllPlayers().filter(p => p.vaccinated)) {
        player.maxImmuneTicks = IMMUNETICKS * VACCINATION_INFECTION_LENGTH_FACTOR
        player.maxIncubationTicks = INCUBATIONTICKS * VACCINATION_INFECTION_LENGTH_FACTOR
    }
})
...
CONFIG_VACCINATION_INFECTION_PROBABILITY_FACTOR.addEventListener("change", event => {
    ...
    for (let player of getAllPlayers().filter(p => p.vaccinated)) {
        player.infectionChance = INFECTION_CHANCE * VACCINATION_INFECTION_CHANCE_FACTOR
    }
})
